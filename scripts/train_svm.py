...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena SVM con split train/val/test sobre features HOG+PCA."
    )
    parser.add_argument(
        "--features-npz",
        type=Path,
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("artifacts/svm_hog_pca_v1"),
...
def build_model(kind: str, c_value: float, gamma_value: str | float, seed: int) -> Pipeline:
    if kind == "linear":
        estimator = LinearSVC(
            C=c_value,
            dual=False,
        ...
        candidates.append(
            {
                ...
                "C": c_value,
                "gamma": "na",
                "name": f"linear_C{c_value:g}",
            }
        )
    for c_value in [1.0, 3.0, 10.0, 30.0]:
        for gamma_value in ["scale", 0.03, 0.01]:
            gamma_str = str(gamma_value).replace(".", "p")
            candidates.append(
                {
                    "kind": "rbf",
                    "C": c_value,
                    "gamma": gamma_value,
                    "name": f"rbf_C{c_value:g}_g{gamma_str}",
                }
            )
    return candidates
...
def plot_selection_scores(selection_df: pd.DataFrame, metric: str, output_path: Path) -> None:
    ...
    ax.set_title(f"Top modelos por {metric} en validation")
...
def main() -> None:
    ...
    print("Entrenando candidatos SVM y seleccionando por validation...")
    ...
    best_model_final = build_model(
        kind=str(best_row["kind"]),
        c_value=float(best_row["C"]),
        gamma_value=best_row["gamma"],
        seed=args.seed,
    )
    ...
    joblib.dump(best_model_final, output_paths["models"] / "svm_best_model.joblib")
